...
;   $0800 -> $09FF - Blob file buffer
;   $0A00 -> $7FFF - User space RAM
;   $8000 -> $9FFF - I/O
;   $A000 -> $FDFF - ROM Code
;   $FE00 -> $FFFB - ROM Jump table
...
ZP_BlobBufPTR = $F4                     ; 2 bytes including $F4,$F5
; Content and display mask for the 7 segment display
ZP_DisplayMask = $D3                    ; 6 bytes including $D3,$D4,$D5,$D6,$D7,$D8
ZP_Display = $D0                        ; 3 bytes including $D0,$D1,$D2
...
RandByte = $0200                        ; Random byte from random number function
RandSeed = $0201                        ; 2 bytes including $0202 - A 16 bit seed value for random function
...
InputPosition = $0207                   ; Which char position of input (address or data)
MonitorMode = $0208                     ; Current mode of monitor ROM
; FAT32 support data
...
SectorCount          = $0237            ; 4 bytes including $0237,$0238,$0239,$023A
ChunkNum             = $0240            ; 2 Bytes including $0240,$0241 - Chunk number to load/save
ChunkCounter         = $0242            ; 2 Bytes including $0242,$0243 - Chunk counter
FATDirty             = $0244            ; Dirty flag for FAT cache.
BlobFirstCluster     = $0245            ; 4 bytes including $0245,$0246,$0247,$0248 - First cluster of Blob file
...
NMI_Vec = $02FC                         ; 2 bytes including $02FC,$02FD - MNI vector in RAM
IRQ_Vec = $02FE                         ; 2 bytes including $02FE,$02FF - IRQ vector in RAM
...
Reset:
  ...
  lda #$FF                              ; $FF is all bits output
  sta DDRB                              ; Set 6522 Port B to all output
  lda #$00                              ; $00 is all bits input
  sta DDRA                              ; Set 6522 Port A to all input
  lda #$10                              ; Set T2 mode to free running - For brightness control
  sta $801B                             ; Set T2 mode to free running - For brightness control
  lda #$00                              ; Set T2 counter to 0 for fastest speed - For brightness control
  sta $8018                             ; Set T2 counter to 0 for fastest speed - For brightness control
  lda #$00                              ; Set output byte to 00, for max brightness - For brightness control
  sta $801A                             ; Set output byte to 00, for max brightness - For brightness control
  ...
  lda #$00                              ; Reset Input Position and Monitor Mode.
  sta InputPosition                     ; Reset input position to $00.
  sta MonitorMode                       ; Reset Monitor Mode. - 00-Idle, 01-Read, 02-Write-Addr, 03-Write-Data, 04-Execute.
  lda #0b01111111                       ; Setup Display Mask, default to digits without the decimal point.
  sta ZP_DisplayMask                    ; Setup Display Mask, default to digits without the decimal point.
  sta ZP_DisplayMask+1                  ; Setup Display Mask, default to digits without the decimal point.
  sta ZP_DisplayMask+2                  ; Setup Display Mask, default to digits without the decimal point.
  sta ZP_DisplayMask+3                  ; Setup Display Mask, default to digits without the decimal point.
  sta ZP_DisplayMask+4                  ; Setup Display Mask, default to digits without the decimal point.
  sta ZP_DisplayMask+5                  ; Setup Display Mask, default to digits without the decimal point.
  lda #0b00001100                       ; Reset SPI bus. set both CS_ lines high, set clock and data low.
  sta PORTB                             ; Reset SPI bus. set both CS_ lines high, set clock and data low.
  cli                                   ; Turn on interrupts to support BRK debug.
  lda #$00                              ; Setup to read "00.BIN"
  sta FileNumber                        ; Set FileNumber to $00
...
MonitorROM:
  ...
  beq MonitorROM                        ; Return to top of Mainloop
CheckReadKey:
  cmp #$FA                              ; Check if Read button pressed
  bne CheckWriteKey                     ; If not Read, then check Write key
  ldx #$01                              ; Set mode to Memory Read
  stx MonitorMode                       ; Set mode to Memory Read
  ldx #$00                              ; Reset input position to 00
  stx InputPosition                     ; Reset input position to 00
  jmp MonitorROM                        ; Return to top of Mainloop
CheckWriteKey:
  cmp #$FB                              ; Check if Write button pressed
  bne CheckExecKey                      ; If not Write, then check Exec key
  ldx #$02                              ; Set Mode to Memory Write
  stx MonitorMode                       ; Set Mode to Memory Write
  ldx #$00                              ; Reset input position to 00
  stx InputPosition                     ; Reset input position to 00
  jmp MonitorROM                        ; Return to top of Mainloop
CheckExecKey:
...
CheckIdle:
  ...
  jmp Idle                              ; Mode is Idle, Jump to Idle
CheckRead:
  cmp #$01                              ; Check Mode with 01/Read
  bne CheckWriteAddr                    ; If not Read, then check if Mode is WriteAddress
  jmp ReadMode                          ; Mode is Read, Jump to Read
CheckWriteAddr:
  cmp #$02                              ; Check Mode with 02/WriteAddress
  bne CheckWriteData                    ; If not WriteAddress, then check if Mode is WriteData
  jmp WriteAddr                         ; Mode is WriteAddress , Jump WriteAddress
CheckWriteData:
  cmp #$03                              ; Check Mode with 03/WriteData
  bne CheckExec                         ; If not WriteData, then check if Mode is Execute
  jmp WriteData                         ; Mode is WriteData, Jump WriteData
CheckExec:
  cmp #$04                              ; Check Mode with 04/Exec
  bne Idle                              ; If not exec, then assume Idle
  jmp Exec                              ; Mode is exec, Jump Exec
Idle:
  jmp MonitorROM                        ; Return to main loop

ReadMode:
  lda KeyCode                           ; Get key pressed
  cmp #$FD                              ; If key pressed is Enter
  beq IncAddress                        ; Increment the Read Address
  jmp AddressInput                      ; If key pressed is not Enter, then assume it's part of a memory address
IncAddress:
  lda #$00                              ; Reset input position to 00
  sta InputPosition                     ; Reset input position to 00
  inc ZP_Display+1                      ; Increment address low byte
  bne ReadModeExit                      ; If ZP_Display+1 is not zero, then exit
  inc ZP_Display+2                      ; Lower byte must have rolled over, so Increment address high byte
ReadModeExit:
  jmp MonitorROM                        ; Return to Mainloop

WriteAddr:
  lda KeyCode                           ; Get key pressed
  cmp #$FD                              ; If key is Enter, then update Mode to WriteData
  bne AddressInput                      ; If Some other key pressed, then process input address
  lda #$00                              ; Reset input position to 00 ready for WriteData input
  sta InputPosition                     ; Reset inout position to 00 ready for WriteData input
  lda #$03                              ; Change Mode to 03/WriteData
  sta MonitorMode                       ; Store the new Mode
  jmp MonitorROM                        ; Return to Mainloop

WriteData:
  lda KeyCode                           ; Get key pressed
  cmp #$FD                              ; If key is Enter, then increment address.
  bne WriteDataInput                    ; If some other key pressed then process input data
  lda #$00                              ; Reset input position to 00
  sta InputPosition                     ; Reset input position to 00
  inc ZP_Display+1                      ; Increment address low byte
  bne WriteDataExit                     ; If ZP_Display+1 is not Zero, then exit 
  inc ZP_Display+2                      ; Lower byte must have rolled over, so Increment address high byte
WriteDataExit:
  jmp MonitorROM                        ; Return to Mainloop
...
DataPosition0:
  lda #$0F                              ; Mask off top half of byte, to keep lower half
  and ZP_Display                        ; Mask off top half of byte, to keep lower half
  sta ZP_Display                        ; Mask off top half of byte, to keep lower half
  lda KeyCode                           ; Get the key pressed
  asl                                   ; Shift left 4 times to get input key to upper half of byte
  asl                                   ; Shift left 4 times to get input key to upper half of byte
  asl                                   ; Shift left 4 times to get input key to upper half of byte
  asl                                   ; Shift left 4 times to get input key to upper half of byte
  ...
  jmp StoreDataToMemory                 ; Go and store the data in Memory pointed to by Address
DataPosition1:
...
AddressInput:
  ...
  beq AddressPosition0                  ; Do input position 0
  cmp #$01                              ; If input position is 1
  beq AddressPosition1                  ; Do input position 1
  cmp #$02                              ; If input position is 2
  beq AddressPosition2                  ; Do input position 2
  cmp #$03                              ; If input position is 3
  beq AddressPosition3                  ; Do input position 3
  jmp MonitorROM                        ; Return to Mainloop - This should never happen!
AddressPosition0:
  ...
  lda KeyCode                           ; Get key pressed
  asl                                   ; Shift left 4 times to get input key to top half of byte
  asl                                   ; Shift left 4 times to get input key to top half of byte
  asl                                   ; Shift left 4 times to get input key to top half of byte
  asl                                   ; Shift left 4 times to get input key to top half of byte
  ora ZP_Display+2                      ; Join new top half byte with old lower half byte
  sta ZP_Display+2                      ; Store new address byte
  inc InputPosition                     ; Increment input position to be ready for next half byte of address
  jmp MonitorROM                        ; Return to Mainloop
AddressPosition1:
...
AddressPosition2:
  ...
  asl                                   ; Shift left 4 times to get input key to top half of byte
  asl                                   ; Shift left 4 times to get input key to top half of byte
  ora ZP_Display+1                      ; Join new top half byte with old lower half byte
...
Exec:
  lda KeyCode                           ; Get key pressed
  cmp #$FD                              ; If key is Enter then do Exec
  beq ExecEnt                           ; Go and do exec
  jmp AddressInput                      ; If some other key pressed we deal with address input
ExecEnt:
...
FindCol:
  ...
  asl                                   ; Col*2
  asl                                   ; Col*2 again to get Col*4
  clc                                   ; Clear carry before add of (Col*4) + KeyRow
  adc KeyRow                            ; Add KeyRow
  tax                                   ; Move A to X so we can use X as index to array
  lda KeypadArray,x                     ; Lookup key code
  rts                                   ; Returns key pressed in A
...
Sleep_loop_Y:                           ; Top of outer loop
  dey                                   ; Decrement Y in outer loop
Sleep_loop_X:                           ; Top of inner loop
  dex                                   ; Decrement X in inner loop
  cpx #$FF                              ; Check if inner loop is done
  bne Sleep_loop_X                      ; Do inner loop again
  cpy #$FF                              ; Check if outer loop is done
  bne Sleep_loop_Y                      ; Do outer loop again
  rts                                   ; Finished 65535 times around the loop
...
Sleep_Short_loop:
  ...
  cpx #$FF                              ; Check if it rolled back to $FF
  bne Sleep_Short_loop                  ; If not done then go around again.
  rts                                   ; Finished 256 times around the loop



UpdateDisplay:
  jsr SPI_Select_7seg                   ; Sets the Chip Select line low for 7 segment display
  jsr Update_7seg                       ; Sends relevent bytes to the display
  jsr SPI_Unselect_7seg                 ; Resets the Chip Select line high for 7 segment display
  rts                                   ; Returns nothing
...
Update_7seg:                            ; Displays three bytes as 6 HEX digits
  ...
  and ZP_DisplayMask                    ; Mask off any segments not to display.
...
SPI_Write_Byte:                         ; Writes the byte in A register to SPI bus.
  sta ZP_Temp                           ; Store Byte in ZeroPage_Temp for speed
  ldx #$08                              ; Setup X as a counter for 8 bits
SPI_loop:
  asl ZP_Temp                           ; Bit shift byte in temp, top bit goes into carry
  bcc SPI_out_low_bit                   ; Check the bit in carry
SPI_out_high_bit:                       ; Outputs a high bit
...
SPI_Read_Byte:                          ; Reads a byte from SPI bus stores in A register.
  ...
  and #0b11111110                       ; Ensure clock is low
...
SendCMD55_ACMD41:
  ...
  jmp SD_Card_Error                     ; Return with error
ACMD41:
  lda #$FF                              ; Setup $FF byte
...
SD_Card_Read_Sector:                    ; Reads a sector to location pointed to by ZP_SectorBufPTR
  jsr Sector_Math                       ; Go do maths. SD is byte indexed, SDHC is sector indexed
...
CheckForSize:                           ; Check if we are at end of bytes to read 
...
ReadJunk:                               ; Read a junk byte
  jsr SPI_Read_Byte                     ; Read a junk byte
  inc ZP_Counter16                      ; Increment counter low byte
...
CMD17_Cleanup:
  ...
  lda #$FF                              ; Setup $FF byte
  jsr SPI_Write_Byte                    ; Send $FF after read finished
  lda #$FF                              ; Setup $FF byte
  jsr SPI_Write_Byte                    ; Send $FF before CS_ change
  jsr SPI_Unselect_SDcard               ; Unselect the SD card.
  lda #$FF                              ; Setup $FF byte
  jsr SPI_Write_Byte                    ; Send $FF after CS_ change
  rts
...
SD_Card_Mount:                          ; Reads Boot Sector and Patition table to setup SDcard.
  ...
  sta CurrentSector                     ; MSB of Sector Number
  sta CurrentSector+1                   ; Next byte of Sector number
  sta CurrentSector+2                   ; Next byte of Sector Number
  sta CurrentSector+3                   ; LSB of Sector Number
  lda #$00                              ; Setup to read sector to memory $0300
  sta ZP_SectorBufPTR                   ; Set buffer start pointer Low byte
  lda #$03                              ; Setup to read sector to memory $0300
  ...
  jsr SD_Card_Read_Sector               ; Read sector 00 (MBR)
  lda $04FE                             ; Check Boot Sector marker at offset $01FE of sector (loaded at $0300)
...
CheckBootSec:
  lda $04FF                             ; Check Boot Sector marker at offset $01FF of Sector (loaded at $0300)
  cmp #$AA                              ; Check boot sector marker of $AA
  beq CheckPatitionType                 ; Continue to check Patition type
  jmp BootSecError                      ; No match means return error
CheckPatitionType:
  lda $04C2                             ; Get patition type code from offset $01C2 of Sector (Loaded at $0300)
...
ReadPartition:
  ...
  clc                                   ; Clear carry before add of LBABegin + ReservedSectors = FAT1BeginLBA
  ...
  clc                                   ; clear carry before add of FAT1BeginLBA + FATsize = FAT2Begin
...
ClusterBegin:                           ; Get Sectors per FAT, then * 2, then add FAT bagin to get ClusterBegin.
  ...
  clc                                   ; Clear carry before adding FAT1Begin + (SectorsPerFAT * 2) = ClusterBegin
...
SD_Card_Error:                          ; Clean up SPI/SDcard things and return an error
  ...
  lda #$FF                              ; Setup $FF junk byte
  jsr SPI_Write_Byte                    ; Send $FF junk byte before CS_ change
  jsr SPI_Unselect_SDcard               ; Ensure SDcard is not selected
  lda #$FF                              ; Setup $FF junk byte
  jsr SPI_Write_Byte                    ; Send $FF junk byte after CS_ change
  rts                                   ; Return an error
...
NextRecord:
  clc                                   ; Clear the carry before we add ZP_DIRRecordPTR + $20 = pointer to next record
...
.IncrementSector
  ...
  clc                                   ; Clear carry before add of SectorIndex + 1
...
GetNextReadCluster:
  ...
  clc                                   ; Clear carry before add of Offset into FAT + $0500 = record in FAT
...
Sector_Math:                            ; SDcard needs byte offset, SDHC needs Sector offset
...
SD_Math:                                ; Standard SD card need sector converted to bytes. (Sector * 512)
  lda CurrentSector+3                   ; Move sector by one whole byte to get Sector * 256
  sta SectorTemp+2                      ; Move sector by one whole byte to get Sector * 256
  lda CurrentSector+2                   ; Move sector by one whole byte to get Sector * 256
  sta SectorTemp+1                      ; Move sector by one whole byte to get Sector * 256
  lda CurrentSector+1                   ; Move sector by one whole byte to get Sector * 256
  sta SectorTemp                        ; Move sector by one whole byte to get Sector * 256
  lda #$00                              ; Move sector by one whole byte to get Sector * 256
  sta SectorTemp+3                      ; Move sector by one whole byte to get Sector * 256
  clc                                   ; Clear carry before the rotate
  rol SectorTemp+3                      ; Rotate left to get a * 2 - Sector * 225 * 2 = Sector * 512 
  rol SectorTemp+2                      ; Rotate left to get a * 2 - Sector * 225 * 2 = Sector * 512 
  rol SectorTemp+1                      ; Rotate left to get a * 2 - Sector * 225 * 2 = Sector * 512
  rol SectorTemp                        ; Rotate left to get a * 2 - Sector * 225 * 2 = Sector * 512
Sector_Math_Done:
  rts
...
LBA_Addr:                               ; Calculates LBA sector from cluster address.
  ...
  lda ClusterNum+3                      ; Cluster LSB
  sbc #$02                              ; Subtract 2 from cluster number because FAT32 doc says so.
  ...
  cmp #$00                              ; Ensure we have a positive number of Sectors per cluster
  bne SectorMultiplyLoop                ; If we have a valid sectors per cluster then do the math
  jmp LBAError                          ; Return an error, Sectors per Cluster can't be 00
...
SectorMultiplyDone:
  ...
  adc ClusterBeginLBA+3                 ; Add ClusterBeginLBA
...
LoadFileLoop:                           ; Top of loop that read file one sector at a time.
  ...
  sta SectorCount                       ; Store Number of whole sectors needed
  ...
  cmp SectorCount                       ; Check number of whole sectors to load
...
LoadWholeSector:                        ; If whole sector to read then read whole sector
  ...
  clc                                   ; Clear carry before add of FileAddrPTR +$0200 = address to load next sector
...
WriteFile:
  jsr FindFile                          ; Check if we have a file with this name already
  ...
  jmp WriteFileError                    ; Don't write file, return error
.SetupWrite
...
WriteFileSector:                        ; Write a whole sector
  ...
  lda FileAddrPTR+1                     ; Add $0200 to Load Address (MSB)
  clc                                   ; Clear carry before add of FileAddrPTR + $0200 = Next sector memory address 
  adc #$02                              ; Add $0200 to file load pointer (MSB)
...
.IncrementSector                        ; Increment the sector number within the current cluster
  ...
  clc                                   ; Clear carry before add of CurrentSector + $01 = next Sector
  lda #$01                              ; Add 01 to Sector
  adc CurrentSector+3                   ; Add to lower byte of sector number
  sta CurrentSector+3                   ; store new sector number
  lda #$00                              ; Load 00 for add to deal with carry
...
FindFreeCluster:                        ; Returns free cluster number to ClusterNum
  ...
  cmp ClusterNum                        ; if ClusterNum = $FF this this is first cluster
...
.IncClusterNum                          ; Increment the pointer and the cluster number
  clc                                   ; Clear carry before add of ClusterNum + $01
  ...
  clc                                   ; Clear carry before add of $04 to record pointer
...
LinkClusterChain:
  ...
  rol ZP_DIRRecordPTR                   ; Mul*2 again (4 bytes per record in the FAT)
  rol ZP_DIRRecordPTR+1                 ; Deal with overflow of mul
  clc                                   ; Clear carry before add of Offset into FAT + $0500 = pointer
  lda ZP_DIRRecordPTR+1                 ; Add $0500 to offset into FAT.
  adc #$05                              ; add $0500 to offset into FAT.
  sta ZP_DIRRecordPTR+1                 ; add $0500 to offset into FAT.
  lda #$00                              ; add $0500 to offset into FAT.
...
.SearchNextRecord                       ; Move up 32 bytes to the next record
  clc                                   ; Clear the carry before we add $20 to Pointer = next record.
  lda ZP_DIRRecordPTR                   ; Get pointer to Directory record
  adc #$20                              ; Add $20 to increment Pointer to next record.
  sta ZP_DIRRecordPTR                   ; Store new directory record pointer low byte
...
WriteFAT:                               ; Writes current Sector of FAT back to disk
  lda FAT1BeginLBA                      ; Copy FAT begin to current sector
  sta CurrentSector                     ; Copy FAT begin to current sector
  lda FAT1BeginLBA+1                    ; Copy FAT begin to current sector
  sta CurrentSector+1                   ; Copy FAT begin to current sector
...
InitBlob:
  sta TempByte                          ; If A is $00 then allocate file but don't write all bytes, $FF == zero fill file.
.ClearMemory
...
.WriteBlobFileLoop                      ; Write the file to SDcard one sector at a time
  lda BytesToWrite+1                    ; Offset BytesToWrite by a whole byte (BytesToWrite/256) to get Sector count
  sta SectorCount                       ; Offset BytesToWrite by a whole byte (BytesToWrite/256) to get Sector count
  lda BytesToWrite+2                    ; Offset BytesToWrite by a whole byte (BytesToWrite/256) to get Sector count
  sta SectorCount+1                     ; Offset BytesToWrite by a whole byte (BytesToWrite/256) to get Sector count
  lda BytesToWrite+3                    ; Offset BytesToWrite by a whole byte (BytesToWrite/256) to get Sector count
  sta SectorCount+2                     ; Offset BytesToWrite by a whole byte (BytesToWrite/256) to get Sector count
  lda #$00                              ; Offset BytesToWrite by a whole byte (BytesToWrite/256) to get Sector count
  sta SectorCount+3                     ; Offset BytesToWrite by a whole byte (BytesToWrite/256) to get Sector count
  clc                                   ; Clear Carry before rotate
  ror SectorCount+3                     ; Sector count / 2 = (BytesToWrite / 512)
  ror SectorCount+2                     ; Sector count / 2 = (BytesToWrite / 512)
  ror SectorCount+1                     ; Sector count / 2 = (BytesToWrite / 512)
  ror SectorCount                       ; Sector count / 2 = (BytesToWrite / 512)
  ...
  lda SectorCount+3                     ; Check if SectorCount = 0
...
Read_Chunk:                             ; read a sector of a file.
  ...
  lda #$08                              ; setup blob buffer at $0800
  sta ZP_BlobBufPTR+1                   ; setup blob buffer at $0800
  lda ChunkNum                          ; Get chunk number
  sta ChunkCounter                      ; Store chunk number to a counter
  lda ChunkNum+1                        ; Get chunk number
  sta ChunkCounter+1                    ; Store chunk number to a counter
...
.CountChunks                            ; loop to move through the file sector chain
  lda ChunkCounter                      ; loop to move through the file sector chain
  ora ChunkCounter+1
  ...
  sec                                   ; Set carry before subtract
  lda ChunkCounter                      ; Load chunk counter
  sbc #$01                              ; Subtract 1
  sta ChunkCounter                      ; Store chunk counter
  lda ChunkCounter+1                    ; load MSB of chunk counter
  sbc #$00                              ; Subtract 0 to deal with any borrow.
  sta ChunkCounter+1                    ; Store MSB of Chunk counter
...
Write_Chunk:                            ; Write a sector of a file.
  ...
  sta ZP_BlobBufPTR+1                   ; Setup blob buffer at $0800
  lda ChunkNum                          ; Get Chunk number
  sta ChunkCounter                      ; Store chunk number to chunk Counter
  lda ChunkNum+1                        ; Get Chunk number
  sta ChunkCounter+1                    ; Store chunk number to Chunk Counter
  lda BlobFirstCluster                  ; Get First Cluster of Blob for searching
  sta ClusterNum                        ; Set Cluster Number for start of search
  lda BlobFirstCluster+1                ; Get First Cluster of Blob for searching
  sta ClusterNum+1                      ; Set Cluster Number for start of search
...
.CountChunks                            ; loop to move through the file sector chain
  ...
  jsr GetNextReadSector                 ; loop to move through the file sector chain
  sec                                   ; Set carry before subtract
  lda ChunkCounter                      ; Load Chunk Counter LSB
  sbc #$01                              ; Subtract 1
  sta ChunkCounter                      ; Store Chunk Counter LSB
  lda ChunkCounter+1                    ; Load Chunk Counter MSB
  sbc #$00                              ; Subtract 0 to deal with any borrow
  sta ChunkCounter+1                    ; Store Chunk Counter MSB
  jmp .CountChunks                      ; loop to move through the file sector chain
.WriteChunk
  lda ZP_BlobBufPTR                     ; Setup to read sector to ZP_BlobBufPTR
  sta ZP_SectorBufPTR                   ; Setup to read sector to ZP_BlobBufPTR
